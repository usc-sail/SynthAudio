...
class ASTModel(nn.Module):
    def __init__(
        ...
        if pretrain_stage == True:
            ...
            if model_size == 'tiny':
                ...
                # self.v = _create_vision_transformer('vit_deit_tiny_distilled_patch16_224', pretrained=False, distilled=True, **model_kwargs)
                ...
                self.v = _create_vision_transformer('vit_deit_base_distilled_patch16_384', pretrained=False, distilled=True)
    ...
    def finetuningavgtok(self, x):
        ...
        for blk_id, blk in enumerate(self.v.blocks):
            if self.finetune_method == "embedding_prompt":
                eee = getattr(self, 'embedding_prompt{}'.format(blk_id)).expand(B, -1, -1)
